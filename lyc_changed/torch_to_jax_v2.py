...
def convert_torch_to_jax(torch_state):
    ...
    if 'proprio_encoder.weight' in torch_state:
        ...
        jax_params['action_out'] = {
            'linear1': {},
            'linear2': {}
        }
        w, b = convert_linear(torch_state['action_decoder.proj.0.0.weight'], torch_state['action_decoder.proj.0.0.bias'])
        jax_params['action_out']['linear1']['kernel'] = w
        jax_params['action_out']['linear1']['bias'] = b
        w, b = convert_linear(torch_state['action_decoder.proj.1.weight'], torch_state['action_decoder.proj.1.bias'])
        jax_params['action_out']['linear2']['kernel'] = w
        jax_params['action_out']['linear2']['bias'] = b
