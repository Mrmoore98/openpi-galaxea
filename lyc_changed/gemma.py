...
def save_array(x):
    """保存数组的回调函数"""
    array = jax.device_get(x)
    with open('probs.pkl', 'ab') as f:
        pickle.dump(array, f)
    return x
...
class Embedder(nn.Module):
    ...
    def encode(self, x):
        ...
        # [yc] for galaxea we don't need to do this
        return x
...
class Attention(nn.Module):
    ...
    def __call__(self, xs, positions, attn_mask, kv_cache):
        ...
        # q *= self.configs[0].head_dim ** -0.5 [yc] for galaxea we don't need to do this
        ...
        # [yc] for galaxea we normalize logits
        logits = logits * (self.configs[0].head_dim ** -0.5)
        ...
        # [yc] for galaxea we add softcap
        # Soft capping
        attn_softclamp: float = 50.0  # default in gemma
        logits = logits / attn_softclamp
        ...
        # [yc] for galaxea we opt to use atten add.
        big_neg = -3.3819763e38  # See gemma/modules.py
        masked_logits = (~attn_mask[:, :, None, :, :])*big_neg + logits
        
        # 在attention计算中使用:
        # result_shape = jax.ShapeDtypeStruct(masked_logits.shape, masked_logits.dtype)
        # jax.experimental.io_callback(save_array, 
        #                               result_shape,
        #                               masked_logits)
...
class Module(nn.Module):
    ...
    def setup(self):
        ...
        # [yc]只对action expert进行final norm
        self.final_norms = [
            None,
            RMSNorm(name=_name("final_norm", 1))
        ]
    ...
    def __call__(
        ...
        *,
        ...
        embedded = jax.tree.map(
            lambda e: e * (e.shape[-1]** 0.5) if e is not None else None,
            embedded
        )
        ...
        for i, e in enumerate(embedded):
            if self.final_norms[i] is not None:
                if e is not None:
                    e = self.final_norms[i](e)
            out_e.append(e)

        return out_e, kv_cache
...
# [yc] 使用新的RoPE实现align with pytorch gemma_rope
def _apply_rope(x, *, positions, max_wavelength=10_000):
    # 获取维度信息
    ...
    # 使用 broadcast_to 替代 expand
    inv_freq_expanded = jnp.broadcast_to(
        inv_freq[jnp.newaxis, :, jnp.newaxis],
        (batch_size, inv_freq.shape[0], 1)
    ).astype(jnp.float32)
    
    # 修改 positions 的形状
    positions_expanded = positions[:, jnp.newaxis, :].astype(jnp.float32)  # [batch_size, 1, seq_len]
    freqs = jnp.matmul(inv_freq_expanded, positions_expanded).transpose(0, 2, 1)
    ...
    # 应用旋转
    result = apply_rotary_pos_emb_jax(x, cos, sin)
...
def apply_rotary_pos_emb_jax(x, cos, sin):
    cos = cos[:, :, jnp.newaxis]
    sin = sin[:, :, jnp.newaxis]
    x = (x * cos) + (rotate_half_jax(x) * sin)
