...
def convert_gemma_mixture(jax_params, torch_params):
    ...
    # Action/Proprio mixture (1024维, 共享权重)
    for i in range(18):
        prefix = f'joint_model.mixtures.action.layers.{i}.'
        q_w = layer_params['attn']['q_einsum_1']['w'][i]  # [8, 1024, 256]
        q_w = einops.rearrange(q_w, 'H D O -> D (H O)')
        
        kv_w = layer_params['attn']['kv_einsum_1']['w'][i]  # [2, 1, 1024, 256]
        kv_w = einops.rearrange(kv_w, 'N H D O -> N D (H O)')
        
        o_w = layer_params['attn']['attn_vec_einsum_1']['w'][i]  # [8, 1024, 256]
        o_w = einops.rearrange(o_w, 'H D O -> (H D) O')
        
        # 转换注意力层
        torch_state[prefix + 'self_attn.q_proj.weight'], _ = convert_linear(q_w)  # 1024维权重
        torch_state[prefix + 'self_attn.k_proj.weight'], _ = convert_linear(kv_w[0])
        torch_state[prefix + 'self_attn.v_proj.weight'], _ = convert_linear(kv_w[1])
        torch_state[prefix + 'self_attn.o_proj.weight'], _ = convert_linear(o_w)
        
        # 转换RMSNorm
        torch_state[prefix + 'input_layernorm.weight'] = convert_array(layer_params['pre_attention_norm_1']['scale'][i])
        torch_state[prefix + 'post_attention_layernorm.weight'] = convert_array(layer_params['pre_ffw_norm_1']['scale'][i])
        
        # 转换MLP
        gate_proj_w = layer_params['mlp_1']['gating_einsum'][i]
        torch_state[prefix + 'mlp.gate_proj.weight'] = convert_array(gate_proj_w[0].T)
        torch_state[prefix + 'mlp.up_proj.weight']   = convert_array(gate_proj_w[1].T)
        torch_state[prefix + 'mlp.down_proj.weight'] = convert_array(layer_params['mlp_1']['linear'][i].T)

    # action final norm
    torch_state['joint_model.mixtures.action.norm.weight'] = convert_array(jax_params['final_norm_1']['scale'])

    # joint_model.mixtures.proprio.layers
    # shared weights so we can copy from action layers
    proprio_state = {}
    for k, v in torch_state.items():
        if 'joint_model.mixtures.action' in k:
            proprio_state[k.replace('joint_model.mixtures.action', 'joint_model.mixtures.proprio')] = v
    torch_state.update(proprio_state)
    
    return torch_state

def convert_pi0_weights(jax_params, torch_params, adapte_proprio_and_action=False):
    """主转换函数"""
    ...
    if adapte_proprio_and_action:
        # 4. 转换proprio编码器
        proj_w = jax_params['state_proj']['kernel']
        proj_b = jax_params['state_proj']['bias']
        torch_state['proprio_encoder.weight'], torch_state['proprio_encoder.bias'] = \
            convert_linear(proj_w, proj_b)
        
        # 5. 转换Action 编码器
        torch_state['action_encoder.linear_1.weight'], torch_state['action_encoder.linear_1.bias'] = \
            convert_linear(jax_params['action_in_proj']['kernel'], jax_params['action_in_proj']['bias'])
        torch_state['action_encoder.linear_2.weight'], torch_state['action_encoder.linear_2.bias'] = \
            convert_linear(jax_params['action_time_mlp_in']['kernel'], jax_params['action_time_mlp_in']['bias'])
        torch_state['action_encoder.linear_3.weight'], torch_state['action_encoder.linear_3.bias'] = \
            convert_linear(jax_params['action_time_mlp_out']['kernel'], jax_params['action_time_mlp_out']['bias'])
    
        # Action decoder
        torch_state['action_decoder.weight'], torch_state['action_decoder.bias'] = \
            convert_linear(jax_params['action_out_proj']['kernel'], jax_params['action_out_proj']['bias'])
...
def load_pi0_weights(jax_params, torch_params, adapte_proprio_and_action=False):
    """加载并转换权重"""
    # 直接使用传入的jax_params
    torch_state = convert_pi0_weights(jax_params, torch_params, adapte_proprio_and_action=adapte_proprio_and_action)
    return torch_state
...
if __name__ == "__main__":
    ...
    adapte_proprio_and_action = True
    ...
    pi0_torch_state = load_pi0_weights(jax_params, torch_params, adapte_proprio_and_action=adapte_proprio_and_action)
    
    # save the torch_state
    torch.save({'model': pi0_torch_state}, "pi0_torch_state_full.pt")